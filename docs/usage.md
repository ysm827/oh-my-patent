...
需要 Node.js 22+、npm 和已配置的 AI 编程宿主。建议使用专门的专利工作区；
...
指定 `--output` 时，生成文件位于 `<dir>/<tool>/`。
使用 `adapt install --prune`（或 `adapt setup --prune`）清理带有生成器归属标记的过时文件；
没有标记的文件会保留。设置 `PLANTUML_SERVER_URL` 可改用私有 PlantUML 服务。
